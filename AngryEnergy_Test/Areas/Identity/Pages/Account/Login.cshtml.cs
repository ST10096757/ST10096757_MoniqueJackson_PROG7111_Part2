...
namespace AngryEnergy_Test.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (passwordResetInfo.NeedsPasswordChange)
                {
                    ...
                    return RedirectToAction("SetPassword", "Login", new { userId = user.Id, email = user.Email });
                }
                else
                {
                    // Standard login process if not temporary password
        ...
        private async Task<PasswordResetInfo> NeedsPasswordReset(IdentityUser user)
        {
            var passwordHash = user.PasswordHash;
            var isTemporary = passwordHash.StartsWith("$TEMP$");
            return new PasswordResetInfo { IsTemporary = isTemporary, NeedsPasswordChange = isTemporary };
        }
    }
}
